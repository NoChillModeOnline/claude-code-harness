...
## Phase 29: `!` コマンド構文 + context:fork 強化 — スキル決定論性とコンテキスト分離

作成日: 2026-04-02
起点: 記事「Claude Codeには"もう一つの設計層"がある」の分析。`!` コマンド構文・`context:fork` + `subagent:` による孫エージェント・LLM Debate パターンの検証
目的: (1) `!` コマンド構文でスキル起動時の決定論的コンテキスト注入を導入し、ターン数を削減 (2) TDD のコンテキスト分離を構造化 (3) セキュリティガードレールを追加

### 背景

- 現在のハーネスは `!` コマンド構文を一切使用していない
- `context: fork` は 7 スキルで利用中だが、`subagent:` フィールドは未使用
- Reviewer は毎回起動後に `git diff` を実行するため 2-3 ターンのウォームアップが発生
- Worker 内の TDD は同一コンテキストで Red/Green を実行しており、コンテキスト汚染のリスクあり
- `!` 構文はプラグイン配布時に任意コマンド実行のセキュリティリスクがある

### 設計原則

1. **`!` コマンド = スキル起動時の確定実行** — モデルの「まず diff を取ろう」判断を不要にする
2. **fork + subagent = 構造的なコンテキスト分離** — TDD の Red/Green が同一コンテキストで汚染されない
3. **セキュリティファースト** — `!` 構文を使うなら、危険コマンドの検出を CI に組み込む

### 優先度マトリクス

| 優先度 | Phase | 内容 | タスク数 | 依存 |
|--------|-------|------|---------|------|
| **Required** | 29.0 | `!` コマンドで harness-review 高速化 | 2 | なし |
| **Required** | 29.1 | `!` コマンドで harness-work のコンテキスト注入 | 2 | なし |
| **Required** | 29.2 | `!` コマンドのセキュリティガードレール | 2 | なし |
| **Recommended** | 29.3 | TDD コンテキスト分離（fork + subagent パターン） | 3 | 29.0 |
| **Optional** | 29.4 | LLM Debate スキル（マルチモデル議論） | 2 | 29.3 |
| **Required** | 29.5 | 統合検証・CHANGELOG | 2 | 29.0〜29.4 |

合計: **13 タスク**

---

### Phase 29.0: `!` コマンドで harness-review 高速化 [P0]

Purpose: Reviewer が起動直後から差分を把握し、即レビュー開始できるようにする。ウォームアップの 2-3 ターンを削減。

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.0.1 | `skills-v3/harness-review/SKILL.md` の Code Review フローに `!` コマンド構文を追加。`!\`git diff ${BASE_REF:-HEAD~1} --stat\`` と `!\`git diff ${BASE_REF:-HEAD~1} \| head -500\`` で起動時に差分をプリロード | SKILL.md に `!` 構文が存在し、Reviewer 起動時に diff 出力がコンテキストに展開される | - | cc:TODO |
| 29.0.2 | `skills/harness-review/SKILL.md`（旧スキル）にも同等の `!` コマンドを追加し、v3 との一貫性を確保 | 旧スキルにも `!` 構文があり、同等の動作をする | 29.0.1 | cc:TODO |

### Phase 29.1: `!` コマンドで harness-work のコンテキスト注入 [P0]

Purpose: Worker が Plans.md の現状と git 状態を起動時に即把握。「まず Plans.md を読もう」の 1 ターンを削減。

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.1.1 | `skills-v3/harness-work/SKILL.md` の Solo モード冒頭に `!\`cat Plans.md 2>/dev/null \| head -100\`` と `!\`git diff --stat HEAD 2>/dev/null \| tail -20\`` を追加 | SKILL.md に `!` 構文が存在し、Plans.md 状態と git diff がプリロードされる | - | cc:TODO |
| 29.1.2 | `skills/harness-work/SKILL.md`（旧スキル）にも同等の `!` コマンドを追加 | 旧スキルにも `!` 構文がある | 29.1.1 | cc:TODO |

### Phase 29.2: `!` コマンドのセキュリティガードレール [P0]

Purpose: プラグイン配布時に `!` 構文内の危険コマンドを CI で検出し、安全性を担保する。

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.2.1 | `tests/validate-plugin.sh` に `!` コマンド安全性チェックを追加。`curl`, `wget`, `rm -rf`, `eval`, `>>/etc/`, 外部 URL 送信を検出して警告 | `validate-plugin.sh` が危険な `!` コマンドを含むスキルを検出し、exit code 1 を返す | - | cc:TODO |
| 29.2.2 | `.claude/rules/skill-editing.md` に `!` コマンド構文の使用ガイドラインを追記。許可するコマンド種別（`cat`, `git diff`, `find`, `head`, `tail`）と禁止コマンド（`curl`, `rm -rf`, `eval` 等）を明記 | ルールファイルに `!` コマンドのセキュリティガイドラインが存在 | - | cc:TODO |

### Phase 29.3: TDD コンテキスト分離（fork + subagent パターン） [P1]

Purpose: TDD の Red Phase と Green Phase を `context: fork` + `subagent:` で構造的に分離し、テスト作成者と実装者のコンテキスト汚染を防止。

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.3.1 | `skills/tdd-red/SKILL.md` を新設。`context: fork` + `subagent: reviewer` で、要件のみからテストを作成する Red Phase スキルを定義。`!` 構文で Plans.md のタスク内容とプロジェクト構造を注入 | スキルが存在し、Reviewer agent が独立コンテキストでテストを作成する | 29.0 | cc:TODO |
| 29.3.2 | `skills/tdd-verify/SKILL.md` を新設。`context: fork` + `subagent: reviewer` で、テスト実行結果を判定する Verify Phase スキルを定義。`!` 構文でテスト実行結果を注入 | スキルが存在し、テスト結果の PASS/FAIL を独立コンテキストで判定する | 29.3.1 | cc:TODO |
| 29.3.3 | `skills-v3/harness-work/SKILL.md` の TDD フェーズ（step 3-4）に `tdd-red` / `tdd-verify` スキルの利用オプションを追記。`--tdd-isolated` フラグで fork+subagent 方式に切り替え可能に | SKILL.md に `--tdd-isolated` オプションが記載され、既存のインライン TDD と選択可能 | 29.3.2 | cc:TODO |

### Phase 29.4: LLM Debate スキル [P2]

Purpose: アーキテクチャ判断やタイブレーク時に、複数モデルの独立見解を構造化して統合判断を支援する。

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.4.1 | `skills/llm-debate/SKILL.md` を新設。`context: fork` で議題を受け取り、`!` 構文で Codex exec による外部モデル見解を注入。Claude が統合レビュアーとして最終判断を出力 | スキルが存在し、`/llm-debate` で議題を渡すとマルチモデル分析レポートが返る | 29.3 | cc:TODO |
| 29.4.2 | `agents-v3/team-composition.md` に LLM Debate スキルの活用ガイドを追記。Lead / Worker からの呼び出しパターンと適用場面（技術選定、タイブレーク、破壊的変更判断）を文書化 | team-composition.md に LLM Debate の活用セクションが存在 | 29.4.1 | cc:TODO |

### Phase 29.5: 統合検証・CHANGELOG [P2]

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 29.5.1 | `validate-plugin.sh` + `check-consistency.sh` 全体検証 | 全検証パス | 29.0〜29.4 | cc:TODO |
| 29.5.2 | CHANGELOG.md [Unreleased] に Phase 29 の変更を記録（今まで/今後 形式） | 変更点が Before/After 形式で記載 | 29.5.1 | cc:TODO |
